package com.ccooy.app3.base.viewstate

sealed class SimpleViewState<out T> : IViewState {

    companion object {
        fun <T> result(result: T): SimpleViewState<T> =
            Result(result)
        fun <T> idle(): SimpleViewState<T> =
            Idle
        fun <T> loading(): SimpleViewState<T> =
            Refreshing
        fun <T> error(error: Throwable): SimpleViewState<T> =
            Error(error)
    }
